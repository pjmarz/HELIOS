...
### Docs
- Sync `docs/index.html` landing page with current stack: removed Cloudflared from the Infrastructure card, "Secure & Automated" feature copy, Key Features bullet, and Security skill chip (migrated off HELIOS to a Raspberry Pi). Added Recyclarr to the Media Stack architecture diagram and Media Management component card.
...
### Removed
- **IPTV Services**: Removed Dispatcharr and IPTV EPG services, including:
  - Docker service definitions and containers
  - Configuration directories (`config/dispatcharr`, `config/epg`)
  - Environment variables (`DISPATCHARR_PORT`, `IPTV_EPG_PORT`)
  - Documentation references in `README.md` and `index.html`
