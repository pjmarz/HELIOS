...
curl -X PUT "${PLEX_URL}/library/sections/7/refresh?X-Plex-Token=${TOKEN}" &>> $LOGFILE
countdown 30

# Start Plex media analysis
echo "Starting Plex media analysis..."
curl -X PUT "${PLEX_URL}/library/sections/7/analyze?X-Plex-Token=${TOKEN}" &>> $LOGFILE
countdown 600

# Start Plex metadata refresh
echo "Starting Plex metadata refresh..."
curl -X PUT "${PLEX_URL}/library/sections/7/refresh?force=1&X-Plex-Token=${TOKEN}" &>> $LOGFILE
