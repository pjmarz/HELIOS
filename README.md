...
- **Environment**: `env.sh` is the repo-local, gitignored source of truth for configuration, auto-loaded by direnv on `cd` (via `.envrc`). The `.env` consumed by Docker Compose is generated from it by `system-verify.sh`.
...
- **Docker socket integration** for Homarr: the container runs as root and reads the mounted Docker socket directly, so the dashboard surfaces live container state without `privileged` (a read-scoped socket proxy is the recommended next hardening step).
