...
# Generate .env for Docker Compose from resolved env.sh values
generate_dotenv() {
    local dotenv="${HELIOS_ROOT}/.env"

    cat > "$dotenv" <<EOF
# Auto-generated by system-verify.sh on $(date -Is)
# Source of truth: env.sh — do not edit this file manually.
...
EOF
    ...
    # Append HOMARR_ENCRYPTION_KEY only if available
    if [ -n "$HOMARR_KEY_VALUE" ]; then
        printf "\n# Homarr Encryption Key\nHOMARR_ENCRYPTION_KEY=%s\n" "$HOMARR_KEY_VALUE" >> "$dotenv"
    fi

    log_color "$GREEN" "✓ Generated .env from env.sh ($(grep -c '=' "$dotenv") variables)"
}
...
check_service_health "seerr" "$SEERR_PORT"
