...
# Define the log file
LOGFILE="/home/peter/Documents/dev/HELIOS/script_logs/virgil-cleanup.log"
...
sudo apt-get --purge autoremove -y | tee -a "$LOGFILE"
check_status "sudo apt-get --purge autoremove"

# Remove orphaned packages
sudo deborphan | xargs sudo apt-get -y remove --purge | tee -a "$LOGFILE"
...
if [ -n "$OLD_CONFIGS" ]; then
    echo "$OLD_CONFIGS" | xargs sudo dpkg --purge | tee -a "$LOGFILE"
...
rm -rf ~/.cache/thumbnails/* | tee -a "$LOGFILE"
check_status "rm -rf ~/.cache/thumbnails/*"

# Clear the trash
rm -rf ~/.local/share/Trash/* | tee -a "$LOGFILE"
