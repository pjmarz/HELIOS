...
- **`env.sh`** — Repo-local, gitignored source of truth: environment variables with `export` statements. Loaded by direnv (`.envrc`) and sourced by `_common.sh`; `system-verify.sh` generates `.env` from it.
